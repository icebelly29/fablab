...
const CameraFeed = (() => {
    ...
    function init() {
        ...
        WS.on('state_change', (data) => {
            // Do not automatically start preview during scan.
            // rpicam-vid (preview) conflicts with rpicam-still (capture).
            if (data.state === 'idle') {
                // We could restart preview here, but let's leave it to user
            }
        });
