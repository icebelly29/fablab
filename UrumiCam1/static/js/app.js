...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const btnSystemStop = document.getElementById('btnSystemStop');
    ...
    btnStartScan.addEventListener('click', () => {
        ...
        TileGrid.reset();
        WS.emit('scan_start', { folder_name: folderName, roi_box: { x1, y1, x2, y2 } });
        LogPanel.addLine(`Scan started: ${folderName} (ROI: ${x1.toFixed(2)},${y1.toFixed(2)} → ${x2.toFixed(2)},${y2.toFixed(2)})`, 'success');
    });
    ...
    btnSystemStop.addEventListener('click', () => {
        if (confirm('EMERGENCY STOP: Halt everything?')) {
            WS.emit('system_stop');
            endScan();
        }
    });
