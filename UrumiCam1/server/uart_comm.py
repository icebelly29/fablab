...
class UARTComm:
    ...
    def connect(self):
        ...
        # Ensure any old resources are cleaned up
        self.disconnect()
        ...
        try:
            ...
            time.sleep(0.3)  # Brief wait for enable to process

            # Flush any stale responses/diagnostics from startup
            self._rx_queue.queue.clear()
            self._flush_rx()
            time.sleep(0.1)

            logger.info("[UART] Motor enable command sent")
            return True
    ...
    def enable_motors(self, state=True):
        ...
        self._send_raw(f"enable all {val}")
        logger.info(f"[UART] Motors {'enabled' if state else 'disabled'}")

    def send_command(self, cmd):
        """Send a raw ASCII command to the Pico."""
        self._send_raw(cmd)
        logger.info(f"[UART] Manual command sent: {cmd}")

    # ── Outgoing Commands (public API — same as before) ────────────────────

    def send_jog(self, dx_steps, dy_steps, sps=800, on_complete=None, on_failed=None):
        """
        Jog the gantry by a RELATIVE step delta (like CutterProd).
        Sends command immediately, no timers or complex state.
        Position is updated locally without waiting for hardware confirmation.
        """
        if self.mock:
            logger.debug(f"[UART MOCK] jog ({dx_steps}, {dy_steps}) @ {sps} sps")
            with self._pos_lock:
                self._pos_x_steps += dx_steps
                self._pos_y_steps += dy_steps
            if on_complete:
                on_complete(self.get_position_mm())
            return

        # Update position immediately (dead reckoning - like CutterProd)
        with self._pos_lock:
            self._pos_x_steps += dx_steps
            self._pos_y_steps += dy_steps
        ...
        # Build command exactly like CutterProd does
        ...
        participants = []
        if abs_dx > 0:
            participants.append((self._motor_x_addr, dx_steps, sps_x))
        if abs_dy > 0:
            participants.append((self._motor_y_addr, dy_steps, sps_y))
        ...
        # Send immediately, no retry logic
        self._send_raw(cmd)
        logger.info(f"[UART] Jog sent: {cmd}")

        # Callback immediately with updated position (no timer)
        if on_complete:
            on_complete(self.get_position_mm())
    ...
    def _execute_move(self, target_x, target_y, sps,
                      ...
                      dx_override=None, dy_override=None, on_failed=None):
        """
        ...
        on_failed   — called if move is rejected
        """
        ...
        logger.info(f"[UART] Executing move: {cmd} (Target: {target_x}, {target_y} | Current: {self._pos_x_steps}, {self._pos_y_steps})")

        ok = self._send_with_retry(cmd)
        if not ok:
            logger.error(f"[UART] Move command rejected after {NOPE_MAX_RETRIES} retries: {cmd}")
            logger.error("[UART] Pico is not responding. Check hardware connection, baud rate, and Pico firmware.")
            if on_failed:
                on_failed()
            if self.on_stall:
                self.on_stall("X")
    ...
    def _send_with_retry(self, cmd):
        ...
        for attempt in range(NOPE_MAX_RETRIES):
            self._flush_rx()  # Clear stale responses before sending
            self._send_raw(cmd)
            response = self._wait_for_response(timeout=0.3)
            if response == "ok":
                if attempt > 0:
                    logger.debug(f"[UART] Got 'ok' on attempt {attempt+1}")
                return True
            elif response == "nope":
                logger.debug(f"[UART] Got 'nope' (attempt {attempt+1}/{NOPE_MAX_RETRIES}) — retrying")
                time.sleep(NOPE_RETRY_DELAY_S)
            elif response is None:
                logger.debug(f"[UART] No response (attempt {attempt+1}/{NOPE_MAX_RETRIES}) — retrying")
                time.sleep(NOPE_RETRY_DELAY_S)
            else:
                logger.debug(f"[UART] Got unexpected response: {response} — retrying")
                time.sleep(NOPE_RETRY_DELAY_S)
        logger.error(f"[UART] Command failed after {NOPE_MAX_RETRIES} retries: {cmd}")
        return False

    def _flush_rx(self):
        """Clear all pending messages from the RX queue and serial buffer."""
        if self._serial and self._serial.is_open:
            try:
                self._serial.reset_input_buffer()
            except Exception as e:
                logger.debug(f"[UART] Failed to reset input buffer: {e}")
    ...
    def _wait_for_response(self, timeout=0.5):
        ...
        Skips over unexpected diagnostic messages from the firmware.
        ...
        while True:
            ...
            try:
                ...
                if line_lower in ("ok", "nope", "ready"):
                    return line_lower
                else:
                    # Skip unsolicited firmware diagnostic messages and keep waiting
                    logger.debug(f"[UART] Skipping unexpected message: {line}")
            except queue.Empty:
                return None
