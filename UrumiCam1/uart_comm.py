...
class UARTComm:
    ...
    def connect(self):
        ...
        try:
            ...
            time.sleep(0.1)  # Brief wait for enable to process

            # Flush any stale responses from startup (enable command sends notification, not "ok")
            self._wait_for_response(timeout=0.2)  # Clear any pending messages
    ...
    def send_jog(self, dx_steps, dy_steps, sps=800, on_complete=None, on_failed=None):
        ...
        on_failed() is called if the move is rejected.
        """
        with self._pos_lock:
        ...
        while True:
            ...
            try:
                ...
                if line_lower in ("ok", "nope", "ready"):
                    ...
                    logger.debug(f"[UART] Unexpected response: {line}")
            except queue.Empty:
                return None
