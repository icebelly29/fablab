...
def is_sitting(track):
    """
    Heuristic to decide if a person is sitting using MoveNet keypoints.
    ...
    Idea:
        - Compute torso vertical length = |shoulder_mid_y - hip_mid_y|.
        - Compute leg vertical length = vertical distance (hip_y - ankle_y) per leg.
        - Sitting => legs are less vertical than standing, so
          leg_vertical / torso_vertical is smaller.
    """
    ...
    # Check we have enough confidence for shoulders/hips/ankles
    needed = [L_SH, R_SH, L_HP, R_HP]
    for idx in needed:
    ...
    if torso_v < 1.0:
        # Tiny or weird pose; don't classify as sitting
        return False
    ...
    if not leg_verticals:
        # No ankles -> can't say sitting
        return False

    leg_v = min(leg_verticals)  # use shorter leg (more "folded")

    ratio = leg_v / torso_v

    # Debug: you can print ratio if tuning
    # print("sitting ratio:", ratio)

    return ratio < SITTING_RATIO_THRESHOLD
...
def main():
    ...
    bench_roi_counters = {}   # tid -> consecutive sitting-in-ROI frames
    ...
    while True:
        ...
        # --- Bench lock logic (with sitting check) ---
        ...
        if locked_target_id is None:
            ...
            for tid, count in bench_roi_counters.items():
                if count >= BENCH_LOCK_FRAMES:
                    ...
                    print(f"[INFO] Locked on ID {tid} (bench sitter)")
                    break
        else:
            # Keep lock until track disappears; no sitting check needed after lock
            pass

        # --- Eyes control ---

        # Default: eyes neutral (straight ahead)
